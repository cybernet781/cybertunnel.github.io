...
<?php $options = get_option('iconiconepro'); ?>
	...
	<?php if( $options['themonic_social_share'] == '1') { // display social sharing buttons() ?>
		<?php if( is_single() || is_page() || $options['iop_social_share_home'] == '1') { // display social sharing buttons on home() ?>
			...
			<?php if($options['themonic_facebook'] == '1') { // display facebook () ?>
				<li><iframe src="//www.facebook.com/plugins/like.php?href=<?php the_permalink(); ?>%2F&amp;send=false&amp;layout=button_count&amp;width=107&amp;show_faces=false&amp;font=arial&amp;colorscheme=light&amp;action=like&amp;height=21" scrolling="no" frameborder="0" style="border:none; overflow:hidden; max-width:128px; height:21px;" allowTransparency="true"></iframe></li>
            <?php } ?>
	...
	<?php endif; ?>		
		...
		<?php if ( get_the_author_meta( 'googleplus' ) ) { ?>
				<a style="text-decoration:none; color:#BB3726;" href="<?php the_author_meta( 'googleplus' ); ?>?rel=author"><i class="fa fa-google-plus"></i></a>
			<?php } // End check for G+ ?>
			
			<?php if ( get_the_author_meta( 'twitter' ) ) { ?>
				<a style="text-decoration:none; color:#33CCFF;" href="<?php the_author_meta( 'twitter' ); ?>"><i class="fa fa-twitter"></i></a>
			<?php } // End check for Twitter ?>
			
			<?php if ( get_the_author_meta( 'facebook' ) ) { ?>
			<a style="text-decoration:none; color:#3B5998;" href="<?php the_author_meta( 'facebook' ); ?>"><i class="fa fa-facebook"></i></a>
